...
- 조회(무비차트)
    ...
    - 영화는 오늘 이후에 일정들을 예매한 내역을 기준으로 조회하다.
    - 예매 내역이 많은 순으로 조회한다. 
...
- 조회(상영시간)
    ...
    - 사용자는 영화, 극장, 날짜를 선택하면 
	  극장 정보(극장 이름, 전체 좌석 수)와 상영시간 별 현재 예매 가능 좌석 수를 조회할 수 있다.
      
- 조회(영화 상세 조회)
	- 사용자는 영화 상세 정보를 조회할 수 있다.
    - 상세 정보는 감독, 배우, 장르, 기본정보, 개봉일, 트레일러, 스틸컷이 있다.
    - 장르는 등록된 장르를 보여주며, 장르는 여러가지일 수 있다.
    - 기본정보에는 관람 나이정보, 런닝 시간(분 단위), 제작국가가 있다.
    - 제작 국가는 여러 국가가 될 수 있다.
    - 감독은 여러 감독이 올 수 있다.
    - 배우는 주연, 조연 구분없이 등록된 배우를 영화에 등록된 배우순으로 조회한다.
...
- 예매
    ...
    - 예매가 완료되면 예매번호가 할당되고, 예매 번호를 통해 예매 좌석과 예매 인원 정보를 조회할 수 있다.
    - 예매시 할인은 없다.
...
- 영화 관리
    ...
    - 영화 정보는 영화에 참여한 제목, 감독, 배우, 장르, 기본정보, 개봉일, 내용, 스틸컷, 트레일러를 입력하여 등록한다.
    - 감독은 등록된 감독 중 여러명 선택이 가능하다.
    - 배우는 등록된 배우 중 여러명 선택이 가능하다.
    - 장르는 등록된 장르 중 여러 장르를 선택할 수 있다.
    - 기본정보에서 연령은 등록된 연령 중 하나를 선택 할 수 있다.
    - 런닝 시간은 분으로 입력한다.
    - 제작 국가는 등록된 국가 중 여러 국가를 선택 할 수 있다.
...
- 상영관 관리
    ...
    - 극장 등록 시 상영관을 추가하는 경우, 상영관 번호(1관,2관 등), 총 좌석수, 좌석 정보를 입력하여 등록한다.
    - 극장 등록 후 상영관을 추가하는 경우, 극장 번호, 상영관 번호, 총 좌석수, 좌석 정보를 입력하여 등록한다.
    - 상영관 번호는 극장의 총 관수보다 작거나 같아야 하고, 같은 극장에 등록된 상영관 번호는 쓸 수 없다.(즉 중복X)
...
- 상영시간 관리
    ...
    - 상영시간은 상영관 정보, 영화, 날짜, 상영 시간을 입력하여 등록할 수 있다.
