...
public class StudentProgram implements Program {
	...
	private void scoreManager() {
		...
		System.out.print("기존 학년 : ");
		int grade = scan.nextInt();
		System.out.print("기존 반 : ");
		int classNum = scan.nextInt();
		System.out.print("기존 번호 : ");
	...
	private void deleteStudent() {
		...
		if(list.remove(std)) {
			...
			System.out.println("등록되지 ㅇ낳은 학생 정보입니다.");
		}
	}
	...
	private void insertStudent() {
		...
		// 입력받은 정보로 인스턴스를 생성 ex)std
		Student std = new Student(grade, classNum, num, name);
		
		// std가 리스트에 있는지 확인해서 없으면 추가
		// contains는 Objects.equals(a,b)를 호출해서 a와b의 클래스가 같으면
		// a.equals(b)를 이용하여 있는지 없는지 확인
		if(!list.contains(std)) {
			list.add(std);
			System.out.println("학생이 등록했습니다.");
			return;
		}
		// 있으면 이미 등록됐다고 알림	
